...
The user's five source JPGs remain unchanged in Downloads. The first two are alternative photos of the same Select Stone order, not two independent purchases. The new image set uses visibly synthetic identifiers and replaces customer references with `RA6 Test Customer 2`, `RA6 Test Customer 3`, and `RA6 Test Customer 4`. **All nine PNG files are generated and visually accepted against the logical manifest.** Actual file hashes, dimensions, prompts, provenance, and individual review notes are recorded in `generated-images.json`. App-model extraction has run. All eight cases return valid results; four cases pass every field check and the four Select variants require supplier correction. See [the acceptance report](../../docs/ra6-acceptance-2026-09-10.md) and retained `evaluation/gemini-results.json`.

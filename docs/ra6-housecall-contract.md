...
**Live Housecall writes require the user's explicit approval.** Implementing RA-6, approving a receipt inside the app, configuring an API key, or naming a customer “test” is not that approval. On 2026-09-10, read-only API verification covered all four test jobs and corrected array query serialization. A separately approved two-write test created and verified one attachment and one $21.00 material on Customer #2. Subsequent standing approval covered only the four verified test customers; complete multi-job/multipage and append verification now passes. General exports remain disabled. See the [acceptance evidence](ra6-acceptance-2026-09-10.md).
...
| Receipt image upload | `POST /jobs/{job_id}/attachments`, multipart `file`; documented HTTP 202, live HTTP 201 also observed | Upload one image at a time. Filename contains receipt, intent, page and SHA-256 bytes digest. The body is binary data from private storage; no public URL upload is used. Both 201 and 202 require readback. |
| Attachment verification | `GET /jobs/{id}?expand[]=attachments`; `attachments` has `id`, `file_name`, `url`, `file_type` | The bracketed query was verified live; the client URL-encodes the brackets. Require explicit expanded collection and exact job ID. Match exact filename to a single provider attachment ID. HTTP 201/202 is only accepted, never completed. |

`JobInputMaterial.unit_cost` explicitly says “Unit in cents.” Its quantity is a number (example `1.51`); the schema does not state a precision limit. Builders preserve historical three-decimal values; dispatch is limited to at most two decimals after the live precision mismatch. Integer-cent unit costs and signed-32-bit extended-cost bounds are validated. Supported values were verified by live readback. Tax is not part of the material payload. Neither invoice line items, selling prices, payments, job status updates nor customer changes are implemented.
...
The published attachment representation has no content hash. Filename plus provider attachment ID verifies the stable upload reference; it is not an independent byte-for-byte download verification. Approved live tests verified filename/material reference preservation, attachment expansion, simple quantity/unit-cost readback, two-job routing, and append preservation of existing rows. Both pages of the fractional receipt were verified at both destinations, but its material export remains partial due to the precision mismatch. The user chose to keep unsupported precision blocked. The original case was closed through audited manual handoff; a separate supported-quantity two-page/two-job case fully passed.

...
Remediation lives in the forward-only files under `supabase/migrations` plus the matching Next.js wiring. On 2026-08-18, the one-time RA-208 history reconciliation was completed dev-first and then in production: both hosted projects now record the same 12 canonical Git migrations through `20260818191408_ra2_audit_and_replay_fixes.sql`, with the 27 project-specific split rows removed from migration metadata. The final migration was applied normally with `db push`, not marked applied through repair. The clean PostgreSQL 17 replay, applied SQL suite, hosted post-checks, and zero-leftover checks passed. See the completed operator record and future drift rules in [migration-history.md](../supabase/migration-history.md). Do not rewrite already-applied files or mark new unapplied SQL as applied.
...
- Applied permission and execution checks live in `supabase/tests/ra2_applied.sql`. `npm test` asserts the file covers approve/retention/purge; `npm run test:applied` executes it.

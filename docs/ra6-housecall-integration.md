...
The [synthetic image pack](../fixtures/ra6/README.md) contains nine generated, visually reviewed PNG files with saved prompts and hashes. App-model extraction of this pack has not run. Live acceptance remains pending: verify exact test job IDs and notification/downstream settings, run and review image extraction and the resulting exact export payloads, obtain explicit user approval for a bounded test run, then verify attachment filename preservation/visibility, material append behavior and existing-row preservation, fractional quantity arithmetic, and all returned external IDs. Mock tests do not prove these provider behaviors.
